...
interface Props {
  /** 외부에서 테마를 직접 넘길 수 있음. 없으면 localStorage에서 읽음 */
  theme?: ThemeKey;
}
...
export default function CreditPurchase({ theme: themeProp }: Props) {
  ...
  const [theme, setTheme]     = useState<ThemeKey>(themeProp ?? "black");

  // 테마 prop이 없으면 localStorage에서 읽기
  useEffect(() => {
    if (themeProp) return;
  ...
  function handleBuy(pack: (typeof PACKS)[number]) {
    ...
    // Polar checkout 라우트로 이동 — 서버에서 리다이렉트 처리
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      {/* 섹션 제목 */}
      <div className="mb-6">
        <h2 className={`text-base font-semibold tracking-wide ${th.text}`}>
          Credits
        </h2>
        <p className={`text-xs mt-1 ${th.sub}`}>
          1 credit = 1 photo download (editorial license)
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="mt-auto space-y-3">
                <p className={`text-lg font-semibold tabular-nums ${th.text}`}>
                  {pack.price}
                </p>
                ...
                <button
                  ...
                  disabled={!!loading}
                  className={`w-full py-2 rounded-sm text-xs font-medium tracking-wide
                  ...
                  {isLoading ? (
                    <span className="flex items-center justify-center gap-2">
                      <span className={`w-3 h-3 border rounded-full animate-spin
                        ${isDark
                          ? isHighlighted ? "border-black/30 border-t-black" : "border-white/20 border-t-white/60"
                          : isHighlighted ? "border-white/30 border-t-white" : "border-black/20 border-t-black/60"
                        }`}
                      />
                      Processing…
                    </span>
